...
// 9、为什么选择使用离线包而不是直接把h5资源嵌套进app内？
//        1、更新灵活性：我们内部是通过将包上传到控制塔，如果app内检测到版本更新直接下载即可，但是嵌套的话，更新必须要更新整个app的版本；
//        2、节省流量：我们是在用户打开h5页面时候去检测并且下载的，如果用户没有查看h5页面不会启动下载，嵌套app是下载时候就是包含了h5资源的包，体积大，下载流量高；
//        3、降低存储压力：app端可以动态管理本地下载的h5资源包，长时间未使用的可以清理掉，但是嵌套h5是在下载时候就占用空间的，用户可能会因为app体积太大而放弃安装或者卸载；
//        4、嵌套h5不够灵活，一套代码只能一个app使用，离线包h5可以同时接入多个app；
// 10、离线包怎么判断下载的包是否完整？
//        看解压工具的反馈，如果解压过程中遇到异常，说明解压失败；
//        比较服务器保存的文件大小和实际下载后的文件大小，如果不一致说明包不完整；
//        打开页面过程中，失败或者超过4s未打开立即切回在线模式；
// 11、监控与日志
//        使用performance API检测离线包中页面的加载性能，例如利用domContentLoadedEventEnd 和loadEventEnd 记录页面dom加载时间和完全加载时间，通过PerformanceObserver 
//     监听资源加载事件来记录每个资源的加载时间、状态等信息；