...
// 15、Babel
//        1、什么是Babel？
//               Babel是一个js转译器，可以把一些比较新的语法，比如es6+转换为es5等可以兼容旧版本浏览器或者环境的语法，来保证在旧版本浏览器或者环境上也能正常运行。
//        2、主要作用？
//               1、语法转换：将es6+等比较新的语法转换成es5等一些对浏览器版本兼容性比较高的语法，例如，使用@babel/plugin-transform-arrow-functions转换箭头函数
//                           使用@babel/plugin-transform-classes转换类语法为es5的构造函数；
//               2、支持新的API：对于一些新的API，例如es6新引入的Promise，旧版浏览器是不支持的，Babel会结合polyfill和core-js来实现新API的功能；
//               3、代码优化：转换过程中会做一些去除空格、注释等操作来缩小代码体积；
//        3、工作原理？
//               Babel转译器的工作流程主要分为三个阶段：解析、转换和生成
//               解析：将源代码转换为AST，也就是抽象语法树，这个过程有两个步骤：词法分析和语法分析；
//                     词法分析就是将代码字符串分解成令牌流，语法分析就是将分解成的令牌流根据语法规则转换为AST；
//               转换：通过插件对AST进行修改，具体包括：插件机制、预设(实际开发中，业务比较复杂，会有很多针对语法转换的处理，预设会将这些常用插件组合在一起，引入预设
//                     可以一次性应用多个插件的功能，简化Babel的配置，比如@babel/preset-env)、AST遍历；
//                     插件机制：Babel使用插件来识别和转换语法，比如转换箭头函数、类语法等；
//                     预设：就是一组插件的集合，比如@babel/preset-env会根据目标环境自动启用需要的插件；
//                     AST遍历：递归遍历AST节点，根据插件逻辑添加、删除、修改节点；
//               生成：最后将修改后的AST转换为目标代码字符串，使用代码生成器，例如@babel/generator，根据AST生成符合目标语法的代码，生成过程中尽量保留原始代码格式；
//        4、使用
//              可以通过.babelrc或者vite插件来配置使用

// 16、v8引擎
//         v8引擎是由Google开发的js引擎，主要用来高效执行js代码，通过即时编译技术将js代码直接转换成机器码，避免传统解释执行的性能损耗，主要用于Chrome浏览器、Node/js环境以及基于
//     Chromium 的浏览器，如Edge、Opra等；
//         作用：
//             1、加速Web应用；
//             2、跨平台能力：使js代码在浏览器和Node环境都能高效运行；
//             3、内存管理优化；
//         1、v8引擎与传统浏览器引擎对比：
//             所有现代引擎在执行js代码之前都需要先执行编译阶段，也就是先对源码进行词法分析和语法分析，生成对应的AST树；
//             早期引擎在生成AST树后，会直接逐行读取并立即执行，不需要编译为字节码或者机器码，所以启动速度比较快，但是运行过程中，对每一行代码都是直接读取解析执行，对于大型复杂项目来说，
//         会有很多重复代码片段，引擎依旧是解析执行，比较消耗性能；
//             而在v8引擎中，在将源码解析为AST树后，会立即将AST编译为字节码(不会全部编译，编译是渐进式的)，解释器开始执行字节码，在执行过程中，对于一些高频代码片段，引擎会将代码段编译为
//         机器码，下次执行到这个代码段时会直接执行机器码，机器码执行效率比字节码要高，但是如果例如函数类型变化、代码逻辑变化等情况会触发去优化，暂停正在执行的机器码，从当前位置开始，
//         回退到解释执行字节码，或者根据新情况来编译机器码；

//        2、怎么判断代码段为热点代码？
//               1、计数器检测：
//                      v8会为不同类型代码片段维护独立计数器，比如函数调用计数器、循环迭代计数器、内联缓存命中次数，当计数器达到预设阈值时触发编译；
//                  不过v8的热点阈值是会根据运行时环境动态调整的，比如高性能设备会降低阈值，更早触发优化、复杂代码比如递归函数会更早标记为热点、对于历史数据根据过去优化效果调整阈值，
//                  防止过度编译、对于类型长期稳定的代码段，及时没达到阈值也可能触发编译、也可以通过内置函数来强制标记热点；
//               2、类型反馈：
//                      v8会检测代码执行模式的稳定型，比如函数多次都以相同参数类型调用(比如多次传入number类型)会优先编译、某段代码因为类型变化频繁触发去优化，那么会降低他的优化优先级；
//               3、总结：
//                      所以最终总结，高频+稳定类型：优先编译为机器码；高频+不稳定类型：可能被编译，但是容易触发去优化(放弃编译好的机器码，回退到解释执行字节码或者重新编译代码)；
//                 低频：解释执行字节码；
//      
//        3、早期浏览器垃圾回收和v8垃圾回收对比
//               1、早期引擎使用引用计数和标记清楚法；
//               2、v8使用分代回收策略；
//                      v8将内存分为新生代和老生代，新生代用来存储一些局部变量、临时对象等，存储的对象存活时间都比较短，而老生代用来存储一些如闭包、Dom引用等存活时间长、内存占用大的对象；
//                      1、新生代：
//                          新生代垃圾回收主要采用Scavenge算法，将空间分为From和To空间，默认空间比例为1：1，我们也可以通过--max-old-space-size来调整，然后从全局变量出发，标记所有
//                      被引用的对象为存活对象，然后将From空间的存活对象复制到To空间，同时也压缩内存来消除碎片化，剩下的对象回收掉，回收后，To空间变成From空间，如果在多次新生代回收后，
//                      对象依然存活就会被晋升到老生代，以及超过新生代一般大小的大对象会直接分配到老生代；
//                          新生代触发回收主要是From空间达到阈值，默认为60%、以及对象空间分配失败情况；不定时触发是因为新生代中对象存活时间短，定时执行会消耗性能，而且js代码执行具有动态性，
//                      对象创建和销毁频率不稳定，基于空间可以更好的适应这种频率；
//                      2、老生代：
//                          老生代垃圾回收主要采用标记清除算法，从全局对象开始标记所有被引用的对象为存活对象，未被标记的对象会被清除，清除过后，可能会产生大量内存碎片，这时将标记的存活对象
//                      全部移动到内存一段，消除内存碎片，为了提高效率，标记过程采用增量标记和并发标记，增量标记就是将标记过程拆分为多个小任务，穿插在代码执行中，减少为主线程的阻塞，并发标记
//                      就是利用多线程并行标记，回收会造成线程停顿，并行可以缩短停顿时间；
//                          老生代回收触发有几种情况，比如新生代晋升到老生代导致老生代达到阈值的、老生代中对象分配空间失败的、以及定时触发和在node中支持的手动调用global.gc触发，
//                      因为老生代中存的都是一些长期存活的对象，难被主动释放，定期回收可以清理潜在的”僵尸对象“，也就是未被使用但也未被标记的对象，避免接近阈值才被回收造成卡顿。
...
//         主要用来监听性能相关的事件，通过监控页面的关键渲染指标，例如FP(首次绘制)、FCP(首次内容绘制)来关注页面性能，用于优化性能和监控；
//         FP：浏览器首次向屏幕绘制像素的时间点；
//         FCP：浏览器首次绘制来自DOM的内容的时间点，这些内容可以是文本、图片、SVG或者非空白的canvas等；
//         实现对FP、FCP的监听：
             const observer = new PerformanceObserver((list) => {
                for (const entry of list) {
                    if(entry.name === 'first-paint') {
                        console.log(`首次绘制时间 (FP): ${entry.startTime} 毫秒`)
                    } else if(entry.name === 'first-contentful-paint') {
                        console.log(`首次内容绘制时间 (FCP): ${entry.startTime} 毫秒`);
                    }
                }
             })
             observer.observe({ entryTypes: ['paint'] });
// 20、MutationObserver
//         主要用来监听DOM的变化，DOM树结构、属性、文本内容等发生变化时，会触发对应的回调，用来处理DOM变化的相应逻辑；
//         具体实现：
//         要观察的目标节点
           ...
           const config = {
                ...
                childList: true, // 表示观察目标节点的直接子节点的添加或者移除
                subtree: true // 表示观察目标节点及其所有后代的变化
           }
           // 当观察到变化时执行的回调函数
           // mutationsList包含了所有变化信息的数组，每个元素都是MutationRecord 对象，该对象记录了具体的变化信息
           // observer当前的MutationObserver 实例
           ...
           // 创建一个新的MutationObserver 实例，并传入回调函数
